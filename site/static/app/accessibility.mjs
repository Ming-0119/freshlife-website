...
#reading-dialog{font:inherit;line-height:1.6;color:var(--ink,#1f2b21);background:var(--surface,#fffdf7);border:1px solid var(--line,#ddd);border-radius:20px;padding:24px;width:min(92vw,42rem,720px);max-height:85dvh;overflow:hidden}
#reading-dialog[open]{display:flex;flex-direction:column}#reading-dialog .reading-body{min-height:0;overflow:auto;overscroll-behavior:contain;padding:4px;scrollbar-gutter:stable}
#reading-dialog .reading-actions{flex-shrink:0;border-top:1px solid var(--line,#ddd);margin-top:16px;padding-top:16px}
#reading-dialog .reading-actions button{border:1px solid var(--line,#ddd);border-radius:12px;background:transparent;color:inherit;white-space:normal;overflow-wrap:anywhere;max-width:100%}
#reading-dialog #reading-done{margin-left:auto;background:var(--green,#0a875e);color:var(--on-green,#fff);border-color:var(--green,#0a875e);font-weight:600}
html[data-theme=dark] #reading-dialog #reading-done{color:var(--on-green,#10271b)}
...
if(theme||language)dialog.querySelector('.reading-body').append(appearance);
...
function apply(save=false){document.documentElement.dataset.readingSize=prefs.size;document.documentElement.classList.toggle('reading-reflow',prefs.size!=='100');document.documentElement.toggleAttribute('data-reading-motion',prefs.motion);size.value=prefs.size;motion.checked=prefs.motion;if(save){try{localStorage.setItem(key,JSON.stringify(prefs));status.textContent=t('设置已保存。','Preferences saved.');}catch{status.textContent=t('已应用。浏览器不允许保存，关闭后可能需要重新设置。','Applied. Storage is unavailable; you may need to set this again.');}}}
