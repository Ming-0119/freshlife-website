...
def main():
    ...
    try:
        ...
        mobile_rhythm = cdp.evaluate("(() => { const p=document.querySelector('.story-progress'); const v=document.querySelector('.vision-grid'); const cards=[...v.querySelectorAll('.vision-card')]; const f=document.querySelector('.footer-bottom'); return { progressDisplay:getComputedStyle(p).display, progressCols:getComputedStyle(p).gridTemplateColumns.split(' ').length, progressRows:new Set([...p.children].map(el=>Math.round(el.getBoundingClientRect().top))).size, visionCols:getComputedStyle(v).gridTemplateColumns.split(' ').length, visionTransparent:cards.every(el=>getComputedStyle(el).backgroundColor==='rgba(0, 0, 0, 0)'), visionPadding:Math.min(...cards.map(el=>parseFloat(getComputedStyle(el).paddingLeft))), visionLine:getComputedStyle(v,'::before').content!=='none', footerDisplay:getComputedStyle(f).display, footerLinks:f.querySelectorAll('.footer-utility a').length, legacyFooter:document.querySelector('.site-footer').textContent.includes('中文优先') }; })()")
        check("手机端四步流程保持完整一行", mobile_rhythm["progressDisplay"] == "grid" and mobile_rhythm["progressCols"] == 4 and mobile_rhythm["progressRows"] == 1, json.dumps(mobile_rhythm))
        check("手机端愿景改为连续时间线", mobile_rhythm["visionCols"] == 1 and mobile_rhythm["visionTransparent"] and mobile_rhythm["visionPadding"] >= 60 and mobile_rhythm["visionLine"], json.dumps(mobile_rhythm))
        check("手机端页脚为紧凑实用导航", mobile_rhythm["footerDisplay"] == "grid" and mobile_rhythm["footerLinks"] == 3 and not mobile_rhythm["legacyFooter"], json.dumps(mobile_rhythm))
        cdp.evaluate("document.querySelector('[data-device-view=\"ipad\"]').click()")
        time.sleep(0.3)
