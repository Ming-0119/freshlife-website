...
- 记录上限：库存与清单各 2500 条，使用记录 10000 条；备份文件上限 20 MB。写入失败不提交界面成功状态。
- 修改后运行 `python3 scripts/build.py`、`node --test tests/web-core.test.mjs tests/web-worker.test.mjs` 和 `node --check site/static/app/app.mjs`。
