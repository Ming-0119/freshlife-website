import {foodMatches,foodID,foodKey,foodSearchMatches} from './food-identity.mjs';
...
export function validateState(s) {
 if (!s || s.format!=='freshlife-web' || ![1,2].includes(s.version) || !Number.isSafeInteger(s.revision) || s.revision<0) fail();
 ...
 for(const x of s.items) if(!locations.includes(x.location) || !validDate(x.expiry) || (x.wholeOnly!==undefined && typeof x.wholeOnly!=='boolean') || (x.wholeOnly===true && !Number.isInteger(x.quantity))) fail();
 ...
 return {format:s.format,version:2,revision:s.revision,items:s.items.map(({id,name,quantity,unit,location,expiry,wholeOnly})=>({id,name,quantity,unit,location,expiry,...(wholeOnly===undefined?{}:{wholeOnly})})),shopping:s.shopping.map(({id,name,quantity,unit})=>({id,name,quantity,unit})),history:s.history.map(({id,name,quantity,unit,kind,at})=>({id,name,quantity,unit,kind,at}))};
}
export function consume(s,id,amount,kind,recordId,at) {
 ...
 if(x.wholeOnly===true && !Number.isInteger(amount)) throw new Error('这件食材按整件使用，请填写整数数量。');
...
export function matchingShopping(items, query='') {
 return items.filter(x=>foodSearchMatches(x.name,query));
}
...
// Read-only evidence: never merge batches, infer package conversions or reduce shopping quantities.
export function matchingStock(items, name) {
 return items.filter(item => foodMatches(item.name, name)).sort((a,b) => a.expiry.localeCompare(b.expiry) || a.id.localeCompare(b.id));
}
export function stockReminder(items, name, base = today()) {
 const matches = matchingStock(items, name);
 if (!matches.length) return '';
 const details = matches.slice(0, 2).map(item => `${item.name} ${item.quantity} ${item.unit}（${status(item.expiry, base).label}）`).join('；');
 return `同类库存：${details}${matches.length > 2 ? `；另有 ${matches.length - 2} 批` : ''}。购买前请核对，清单数量不会自动改变。`;
}

export function shoppingStockReminders(items, shopping, base = today()) {
 const group = name => foodID(name) ?? `name:${foodKey(name)}`;
 const groups = new Map(), notes = new Map(), result = new Map();
 for (const item of items) { const key = group(item.name); if (!groups.has(key)) groups.set(key, []); groups.get(key).push(item); }
 for (const item of shopping) {
  const key = group(item.name);
  if (!notes.has(key)) notes.set(key, stockReminder(groups.get(key) ?? [], item.name, base));
  result.set(item.id, notes.get(key));
 }
 return result;
}
